...
void set_srk_rsa_bits(const unsigned int rsa_bits) {
  RSA_SRK_template.publicArea.parameters.rsaDetail.keyBits = rsa_bits;
}

static inline const tpm_evdigest_t *
tpm_evdigest_from_TPM2B_DIGEST(const TPM2B_DIGEST *td, tpm_evdigest_t *result,
...
static bool write_digest(const char *path, const TPM2B_DIGEST *d) {
  buffer_t *bp;
  TPM2_RC rc;
  bool ok = false;
...
static TPM2B_DIGEST *read_digest(const char *path) {
  TPM2B_DIGEST *d;
  buffer_t *bp;
  TPM2_RC rc;
...
static TPM2B_SENSITIVE_DATA *read_secret(const char *path) {
...
static bool write_sealed_secret(const char *path, const TPM2B_PUBLIC *pub,
...
static bool read_sealed_secret(const char *path, TPM2B_PUBLIC **pub_ret,
                               TPM2B_PRIVATE **priv_ret) {
  ...
  if (tss_check_error(rc, "Tss2_MU_TPM2B_MAX_BUFFER_Unmarshal failed")) {
    ...
    *pub_ret = pub;
    ...
    error("%s does not seem to contain a valid pair of public/private sealed "
          "data\n",
          path);
    free(priv);
    free(pub);
...
static void free_secret(TPM2B_SENSITIVE_DATA *sd) {
  memset(sd, 0, sizeof(*sd));
  free(sd);
}

static bool write_signature(const char *path, const TPMT_SIGNATURE *s) {
  buffer_t *bp;
  int rc;
  bool ok = false;
...
static bool read_signature(const char *path, TPMT_SIGNATURE **ret) {
  TPMT_SIGNATURE *s;
  buffer_t *bp;
  int rc;
  bool ok = false;
...
bool tss_write_public_key(const char *path, const TPM2B_PUBLIC *s) {
  buffer_t *bp;
  int rc;
  bool ok = false;
...
TPM2B_PUBLIC *tss_read_public_key(const char *path) {
...
static bool esys_start_auth_session(ESYS_CONTEXT *esys_context,
                                    TPM2_SE session_type,
                                    ESYS_TR *session_handle_ret) {
  static const TPMT_SYM_DEF symmetric = {.algorithm = TPM2_ALG_AES,
                                         .keyBits = {.aes = 128},
                                         .mode = {.aes = TPM2_ALG_CFB}};
  TSS2_RC rc;

  rc = Esys_StartAuthSession(esys_context, ESYS_TR_NONE, /* tpmKey */
                             ESYS_TR_NONE,               /* bind */
                             ...
                             &symmetric, TPM2_ALG_SHA256, session_handle_ret);

  return tss_check_error(rc, "Esys_StartAuthSession failed");
}

static void esys_flush_context(ESYS_CONTEXT *esys_context,
                               ESYS_TR *session_handle_p) {
  TSS2_RC rc;
...
static bool __pcr_selection_build(TPML_PCR_SELECTION *sel,
...
static bool pcr_bank_to_selection(TPML_PCR_SELECTION *sel,
                                  const tpm_pcr_bank_t *bank) {
  return __pcr_selection_build(sel, bank->valid_mask, bank->algo_info);
}

static void __pcr_selection_add(TPML_PCR_SELECTION *sel, unsigned int algo_id,
...
static bool __pcr_bank_hash(ESYS_CONTEXT *esys_context,
                            const tpm_pcr_bank_t *bank, TPM2B_DIGEST **hash_ret,
  ...
  debug("%s: going to hash PCRs from bank %s (TCG algo id %u)\n", __func__,
        bank->algo_info->openssl_name, bank->algo_info->tcg_id);

  rc = Esys_HashSequenceStart(esys_context, ESYS_TR_NONE, ESYS_TR_NONE,
                              ESYS_TR_NONE, &null_auth, bank->algo_info->tcg_id,
                              &sequence_handle);
  ...
  for (i = 0; i < PCR_BANK_REGISTER_MAX; ++i) {
    ...
    rc = Esys_SequenceUpdate(esys_context, sequence_handle, ESYS_TR_PASSWORD,
                             ESYS_TR_NONE, ESYS_TR_NONE, &pcr_value);
  ...
  rc = Esys_SequenceComplete(esys_context, sequence_handle, ESYS_TR_PASSWORD,
                             ESYS_TR_NONE, ESYS_TR_NONE, NULL, esys_tr_rh_null,
                             hash_ret, NULL);
...
failed:
  if (sequence_handle != ESYS_TR_NONE)
    Esys_SequenceComplete(esys_context, sequence_handle, ESYS_TR_PASSWORD,
                          ESYS_TR_NONE, ESYS_TR_NONE, NULL, esys_tr_rh_null,
                          NULL, NULL);
...
static bool esys_policy_pcr(ESYS_CONTEXT *esys_context,
                            TPML_PCR_SELECTION *pcrSel, TPM2B_DIGEST *pcrDigest,
                            TPM2B_DIGEST **result) {
  ...
  rc = Esys_PolicyPCR(esys_context, session_handle, ESYS_TR_NONE, ESYS_TR_NONE,
                      ESYS_TR_NONE, pcrDigest, pcrSel);
...
static TPM2B_DIGEST *__pcr_policy_make(ESYS_CONTEXT *esys_context,
                                       const tpm_pcr_bank_t *bank) {
...
static bool esys_create_authorized_policy(ESYS_CONTEXT *esys_context,
                                          TPM2B_DIGEST *pcrPolicy,
                                          const TPM2B_PUBLIC *pubKey,
  ...
  rc = Esys_LoadExternal(esys_context, ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                         NULL, pubKey, esys_tr_rh_owner, &pub_key_handle);
  ...
  rc = Esys_PolicyAuthorize(esys_context, session_handle, ESYS_TR_NONE,
                            ESYS_TR_NONE, ESYS_TR_NONE, pcrPolicy,
                            &policy_qualifier, public_key_name, &check_ticket);
  if (!tss_check_error(rc, "Esys_PolicyAuthorize failed"))
    goto out;

  /* Now get the digest and return it */
  rc = Esys_PolicyGetDigest(esys_context, session_handle, ESYS_TR_NONE,
                            ESYS_TR_NONE, ESYS_TR_NONE, authorizedPolicy);
  if (!tss_check_error(rc, "Esys_PolicyGetDigest failed"))
    goto out;
...
static bool esys_create_primary(ESYS_CONTEXT *esys_context,
                                ESYS_TR *handle_ret) {
  ...
  rc = Esys_CreatePrimary(esys_context, esys_tr_rh_owner, ESYS_TR_PASSWORD,
                          ...
                          SRK_template, NULL, &creation_pcr, handle_ret, NULL,
                          NULL, NULL, NULL);
...
static bool esys_create(ESYS_CONTEXT *esys_context, ESYS_TR srk_handle,
                        TPM2B_DIGEST *authorized_policy,
                        TPM2B_SENSITIVE_DATA *secret,
  ...
  TPML_PCR_SELECTION creation_pcr = {.count = 0};
  rc = Esys_Create(esys_context, srk_handle, ESYS_TR_PASSWORD, ESYS_TR_NONE,
                   ESYS_TR_NONE, &in_sensitive, &in_public, NULL, &creation_pcr,
...
static bool esys_seal_secret(const target_platform_t *platform,
                             ESYS_CONTEXT *esys_context, TPM2B_DIGEST *policy,
                             const TPML_PCR_SELECTION *pcr_sel,
  ...
  if (!esys_create(esys_context, srk_handle, policy, secret, &sealed_private,
                   &sealed_public))
    goto cleanup;

  ok = platform->write_sealed_secret(output_path, pcr_sel, sealed_private,
                                     sealed_public);
  if (ok)
...
static bool esys_unseal_pcr_policy(ESYS_CONTEXT *esys_context,
                                   const tpm_pcr_bank_t *bank,
                                   const TPM2B_PUBLIC *sealed_public,
                                   const TPM2B_PRIVATE *sealed_private,
  ...
  rc = Esys_Load(esys_context, primary_handle, ESYS_TR_PASSWORD, ESYS_TR_NONE,
                 ESYS_TR_NONE, sealed_private, sealed_public,
  ...
  rc = Esys_PolicyPCR(esys_context, session_handle, ESYS_TR_NONE, ESYS_TR_NONE,
                      ESYS_TR_NONE, &empty_digest, &pcrs);
  if (!tss_check_error(rc, "Esys_PolicyPCR failed"))
    goto cleanup;

  rc = Esys_Unseal(esys_context, sealed_object_handle, session_handle,
...
static bool esys_unseal_authorized(ESYS_CONTEXT *esys_context,
                                   const tpm_pcr_bank_t *bank,
  ...
  if (policy_signature->sigAlg != TPM2_ALG_RSASSA &&
      policy_signature->sigAlg != TPM2_ALG_ECDSA)
    warning("%s: bad sigAlg %x\n", __func__, policy_signature->sigAlg);
  if (__TPMT_SIGNATURE_get_hash_alg((TPMT_SIGNATURE *)policy_signature) !=
      TPM2_ALG_SHA256)
    warning("%s: bad hash %x\n", __func__,
            __TPMT_SIGNATURE_get_hash_alg((TPMT_SIGNATURE *)policy_signature));

  pcr_bank_to_selection(&pcrs, bank);

  rc = Esys_LoadExternal(esys_context, ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                         NULL, pub_key, esys_tr_rh_owner, &pub_key_handle);
  if (!tss_check_error(rc, "Esys_LoadExternal failed"))
    goto cleanup;
  ...
  rc = Esys_Hash(esys_context, ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                 (const TPM2B_MAX_BUFFER *)pcr_policy, TPM2_ALG_SHA256,
                 esys_tr_rh_null, &pcr_policy_hash, NULL);
  if (!tss_check_error(rc, "Esys_Hash failed"))
    goto cleanup;

  rc = Esys_VerifySignature(esys_context, pub_key_handle, ESYS_TR_NONE,
                            ESYS_TR_NONE, ESYS_TR_NONE, pcr_policy_hash,
                            policy_signature, &verification_ticket);
  if (!tss_check_error(rc, "Esys_VerifySignature failed"))
    goto cleanup;

  TPM2B_NONCE policyRef = {.size = 0};
  rc = Esys_PolicyAuthorize(esys_context, session_handle, ESYS_TR_NONE,
                            ESYS_TR_NONE, ESYS_TR_NONE, pcr_policy, &policyRef,
...
static bool __pcr_policy_sign(const tpm_key_t *asym_key,
                              const TPM2B_DIGEST *authorized_policy,
                              TPMT_SIGNATURE **signed_policy) {
  ...
  *signed_policy = NULL;
  ...
  if (!tpm_key_sign(asym_key, authorized_policy->buffer,
                    authorized_policy->size, result)) {
...
static bool __pcr_policy_create_authorized(
    ESYS_CONTEXT *esys_context, const tpm_pcr_selection_t *pcr_selection,
    const stored_key_t *private_key_file, TPM2B_DIGEST **ret_digest_p) {
   ...
   * interested in. */
  pcr_bank_initialize(&zero_bank, pcr_selection->pcr_mask,
                      pcr_selection->algo_info);
  pcr_bank_init_from_zero(&zero_bank);
  if (!(pcr_policy = __pcr_policy_make(esys_context, &zero_bank)))
    goto out;

  okay = esys_create_authorized_policy(esys_context, pcr_policy, pub_key,
                                       ret_digest_p);
 ...
 */
bool pcr_read_into_bank(tpm_pcr_bank_t *bank) {
   ...
   * chunks of 8 or less.
   */
  for (pcr_chunk_offset = 0; pcr_chunk_offset < PCR_BANK_REGISTER_MAX;
       pcr_chunk_offset += 8) {
    unsigned int pcr_mask;
    ...
    for (index = 0, k = 0; index < PCR_BANK_REGISTER_MAX; ++index) {
      ...
      if (d->size != bank->algo_info->digest_size) {
        error("Esys_PCR_Read returns a %s digest with size %u (expected %u)\n",
              bank->algo_info->openssl_name, d->size,
 ...
 */
bool pcr_store_public_key(const stored_key_t *private_key_file,
                          const stored_key_t *public_key_file) {
  tpm_key_t *pub_key;
  bool okay = false;
...
bool pcr_seal_secret(const target_platform_t *platform,
                     const tpm_pcr_bank_t *bank, const char *input_path,
...
static bool pcr_unseal_secret_pcr(const tpm_pcr_selection_t *pcr_selection,
  ...
  pcr_bank_initialize(&pcr_current_bank, pcr_selection->pcr_mask,
                      pcr_selection->algo_info);
  pcr_bank_init_from_current(&pcr_current_bank);

  /* Now we've got all the ingredients we need. Go for it. */
  okay = esys_unseal_pcr_policy(esys_context, &pcr_current_bank, sealed_public,
                                sealed_private, &unsealed);
...
bool pcr_authorized_policy_create(const tpm_pcr_selection_t *pcr_selection,
  ...
  ok = __pcr_policy_create_authorized(esys_context, pcr_selection,
                                      private_key_file, &authorized_policy);
...
bool pcr_authorized_policy_seal_secret(const target_platform_t *platform,
...
bool old_pcr_authorized_policy_seal_secret(const char *authpolicy_path,
                                           const char *input_path,
                                           const char *output_path) {
  ESYS_CONTEXT *esys_context = tss_esys_context();
  TPM2B_DIGEST *authorized_policy = NULL;
  ...
  if (!esys_create(esys_context, srk_handle, authorized_policy, secret,
...
/*
 * The "signing" part of using authorized policies consists of hashing together
 * the set of expected PCR values, and signing the resulting digest.
 */
bool pcr_policy_sign(const target_platform_t *platform,
                     const tpm_pcr_bank_t *bank,
                     const stored_key_t *private_key_file,
                     const char *input_path, const char *output_path,
                     const char *policy_name) {
  ...
  okay =
      platform->write_signed_policy(input_path, output_path, policy_name, bank,
                                    pcr_policy, asym_key, signed_policy);
  if (okay)
 ...
 */
static bool pcr_authorized_policy_unseal_secret(
    const tpm_pcr_selection_t *pcr_selection, const char *signed_policy_path,
    const stored_key_t *public_key_file, const char *input_path,
    const char *output_path) {
  ESYS_CONTEXT *esys_context = tss_esys_context();
  tpm_pcr_bank_t pcr_current_bank;
  ...
  /* Now we've got all the ingredients we need. Go for it. */
  okay =
      esys_unseal_authorized(esys_context, &pcr_current_bank, policy_signature,
                             pub_key, sealed_public, sealed_private, &unsealed);
...
static bool __pcr_policy_tpm2_policyauthorize(ESYS_CONTEXT *esys_context,
                                              ESYS_TR session_handle,
                                              buffer_t *bp) {
  ...
  rc = Tss2_MU_TPM2B_DIGEST_Unmarshal(bp->data, bp->size, &bp->rpos,
                                      &policy_ref);
  if (rc != TSS2_RC_SUCCESS)
    return false;

  rc = Tss2_MU_TPMT_SIGNATURE_Unmarshal(bp->data, bp->size, &bp->rpos,
                                        &policy_signature);
  if (rc != TSS2_RC_SUCCESS)
    return false;
  ...
  rc = Esys_Hash(esys_context, ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                 (const TPM2B_MAX_BUFFER *)pcr_policy, sig_hash_alg,
                 esys_tr_rh_null, &pcr_policy_hash, NULL);
  if (!tss_check_error(rc, "Esys_Hash failed"))
    goto cleanup;
  ...
  rc = Esys_PolicyAuthorize(esys_context, session_handle, ESYS_TR_NONE,
                            ESYS_TR_NONE, ESYS_TR_NONE, pcr_policy, &policy_ref,
                            public_key_name, verification_ticket);
  if (!tss_check_error(rc, "Esys_PolicyAuthorize failed"))
    goto cleanup;
...
static bool __pcr_policy_tpm2_policypcr(ESYS_CONTEXT *esys_context,
                                        ESYS_TR session_handle, buffer_t *bp) {
  ...
  rc = Tss2_MU_TPML_PCR_SELECTION_Unmarshal(bp->data, bp->size, &bp->rpos,
                                            &pcrs);
  if (rc != TSS2_RC_SUCCESS)
    return false;

  rc = Esys_PolicyPCR(esys_context, session_handle, ESYS_TR_NONE, ESYS_TR_NONE,
                      ESYS_TR_NONE, &digest, &pcrs);
  if (!tss_check_error(rc, "Esys_PolicyPCR failed"))
    return false;
...
static bool __pcr_policy_unseal_policy_seq(
    ESYS_CONTEXT *esys_context, ESYS_TR sealed_object_handle,
    STACK_OF(TSSOPTPOLICY) * policy_seq, TPM2B_SENSITIVE_DATA **sensitive_ret) {
  ESYS_TR session_handle = ESYS_TR_NONE;
  TSSOPTPOLICY *policy;
  int i, num_commands;
  TPM2_RC rc;
  ...
  for (i = 0; i < num_commands; i++) {
    ...
    buffer_init_read(&buf, policy->CommandPolicy->data,
                     policy->CommandPolicy->length);
    switch (code) {
    ...
    case TPM2_CC_PolicyAuthorize:
      if (!__pcr_policy_tpm2_policyauthorize(esys_context, session_handle,
                                             &buf))
        goto cleanup;
      break;
    default:
  ...
  rc = Esys_Unseal(esys_context, sealed_object_handle, session_handle,
                   ESYS_TR_NONE, ESYS_TR_NONE, sensitive_ret);
  if (!tss_check_error(rc, "Esys_Unseal failed"))
    goto cleanup;
...
static bool tpm2key_unseal_secret(const char *input_path,
                                  const char *output_path,
                                  const tpm_pcr_selection_t *pcr_selection,
                                  const char *signed_policy_path,
                                  const stored_key_t *public_key_file) {
  ...
  rc = Esys_Load(esys_context, primary_handle, ESYS_TR_PASSWORD, ESYS_TR_NONE,
                 ESYS_TR_NONE, &priv, &pub, &sealed_object_handle);
  if (!tss_check_error(rc, "Esys_Load failed"))
    goto cleanup;
...
bool pcr_unseal_secret(const target_platform_t *platform,
                       const tpm_pcr_selection_t *pcr_selection,
                       const char *signed_policy_path,
                       const stored_key_t *public_key_file,
                       const char *input_path, const char *output_path) {
  ...
  return platform->unseal_secret(input_path, output_path, pcr_selection,
 ...
 */
static bool oldgrub_write_sealed_secret(const char *pathname,
                                        const TPML_PCR_SELECTION *pcr_sel,
                                        const TPM2B_PRIVATE *sealed_private,
                                        const TPM2B_PUBLIC *sealed_public) {
  /* Just marshal public and private portions and concat them into a single
   * file. */
  return write_sealed_secret(pathname, sealed_public, sealed_private);
}

static bool oldgrub_write_signed_policy(
    const char *input_path, const char *output_path, const char *policy_name,
    const tpm_pcr_bank_t *bank, const TPM2B_DIGEST *pcr_policy,
    const tpm_key_t *signing_key, const TPMT_SIGNATURE *signed_policy) {
  /* Just write the signature, that's all */
  return write_signature(output_path, signed_policy);
}

static bool oldgrub_unseal_secret(const char *input_path,
                                  const char *output_path,
                                  const tpm_pcr_selection_t *pcr_selection,
                                  const char *signed_policy_path,
                                  const stored_key_t *public_key_file) {
  ...
  return pcr_authorized_policy_unseal_secret(pcr_selection, signed_policy_path,
                                             public_key_file, input_path,
                                             output_path);
}
 ...
 */
static bool tpm2key_write_sealed_secret(const char *pathname,
                                        const TPML_PCR_SELECTION *pcr_sel,
                                        const TPM2B_PRIVATE *sealed_private,
                                        const TPM2B_PUBLIC *sealed_public) {
  TSSPRIVKEY *tpm2key = NULL;
...
static bool tpm2key_write_signed_policy(
    const char *input_path, const char *output_path, const char *policy_name,
    const tpm_pcr_bank_t *bank, const TPM2B_DIGEST *pcr_policy,
    const tpm_key_t *signing_key, const TPMT_SIGNATURE *signed_policy) {
  TSSPRIVKEY *tpm2key = NULL;
  ...
  if (!tpm2key_add_authpolicy_policyauthorize(tpm2key, policy_name, &pcr_sel,
                                              pub_key, signed_policy, false))
    goto out;
...
static bool systemd_write_signed_policy(
    const char *input_path, const char *output_path, const char *policy_name,
    const tpm_pcr_bank_t *bank, const TPM2B_DIGEST *pcr_policy,
    const tpm_key_t *signing_key, const TPMT_SIGNATURE *signed_policy) {
  ...
  if (signed_policy->sigAlg == TPM2_ALG_RSASSA) {
    ...
    sig_size = signed_policy->signature.ecdsa.signatureR.size +
               signed_policy->signature.ecdsa.signatureS.size;
    memcpy(sig_buffer, signed_policy->signature.ecdsa.signatureR.buffer,
           signed_policy->signature.ecdsa.signatureR.size);
    memcpy(sig_buffer + signed_policy->signature.ecdsa.signatureR.size,
           signed_policy->signature.ecdsa.signatureS.buffer,
           signed_policy->signature.ecdsa.signatureS.size);
  }

  okay = sdb_policy_file_add_entry(output_path, policy_name, bank->algo_name,
                                   bank->pcr_mask,
                                   /* fingerprint */
...
const target_platform_t *pcr_get_target_platform(const char *name) {
...
unsigned int target_platform_unseal_flags(const target_platform_t *platform) {
